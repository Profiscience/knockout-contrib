...
async function checkIntegrity() {
  return napaIntegrityCheck() && (await yarnIntegrityCheck())
}
...
function napaIntegrityCheck() {
  const pkg = require(path.join(rootPath, 'package.json'))

  for (const d in pkg.napa) {
    if (!fs.existsSync(path.join(rootPath, `node_modules/${d}`))) {
      return false
    }
  }
